...
It will be utilized 3 different Machine Learning models. Additionally, it will make model tuning and hyperparameter assessment in this study. These are;
...
fit <- lm( Exited ~ . , data = split_df$train)

rmse (model = fit, data = split_df$test )
...
fit <- rpart( Exited ~ ., data = split_df$train)
...
fit <- gbm( Exited ~ ., distribution = "bernoulli", data = split_df$train)
...
We created 3 different models for the dataset after data preprocessing. All models are observed to have low success prediction rate. It is assessed that dataset has data quality issue. 

(Note: This dataset is created to be used in data analysis studies. So it is not represent a real data and it may be normal not to be found patterns of customers in the dataset. Pattern is vital for ML Algorithms to make correct prediction.)


# 6.CONCLUSION

The purpose of this project is to display R Programming, Statistics and Machine Learning skills rather then catching a high success rate in model. In addition , it was put explanations for several details that was not applied. Several irrelevant data manipulation is made to show technical skills in this study. Besides, basic data science and reporting skills are tried to exhibited in very short study time.
